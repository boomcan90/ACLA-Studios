...
public class B2WorldCreator {
    ...
    public B2WorldCreator(PlayScreen screen) {
        ...
        for (MapObject object : map.getLayers().get(4+(screen.getUserID()/screen.getNumOfPlayers())).getObjects().getByType(PolylineMapObject.class)) {
            float vertices[] = ((PolylineMapObject) object).getPolyline()
                    .getTransformedVertices();
            for(int i = 0; i< vertices.length;i++){
                vertices[i] *= SpaceConquest.MAP_SCALE;
        ...
        for (MapObject object : map.getLayers().get(2+(screen.getUserID()/screen.getNumOfPlayers())).getObjects().getByType(PolylineMapObject.class)) {
            float vertices[] = ((PolylineMapObject) object).getPolyline()
                    .getTransformedVertices();
            for(int i = 0; i< vertices.length;i++){
                vertices[i] *= SpaceConquest.MAP_SCALE;
